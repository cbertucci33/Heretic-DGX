...
def inspect_lora_target_topologies(
    ...
    *,
    ...
    """Fail closed unless every target has a provably safe PEFT TP layout.

    The decision is based solely on metadata attached by Transformers to the
    loaded modules. Model names and architecture-specific projection names are
        ...
        if dtensor_topology is not None:
            ...
            if dtensor_topology == "replicated":
                if model_plan is not None and model_plan != dtensor_topology:
                    raise ValueError(
                        f"tensor-parallel plan disagrees with DTensor placement for "
            ...
            if model_plan != dtensor_topology:
                raise ValueError(
                    f"tensor-parallel plan disagrees with DTensor placement for LoRA "
...
def directional_lora_factors(
    ...
    *,
    ...
    """Compute local directional factors for a supported TP weight layout.

    Rowwise PRE normalization needs a global sum of squared row fragments.
    Colwise layouts own complete rows but always need a global sum of each
    rank's v^T W contribution. The base weight is read only.
    """
