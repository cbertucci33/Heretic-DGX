...
class Settings(BaseSettings):
    ...
    cluster: str | None = Field(
        default=None,
        description="Path to a two-node DGX cluster TOML file.",
        exclude=True,
    )
    ...
    startup_check: bool = Field(
        default=False,
        description=(
            "Load the model, initialize Heretic adapters, run one collective text "
            "response, and exit without analysis, ablation, or export."
        ),
        exclude=True,
    )
...
def _config_path_from_argv(argv: list[str]) -> str:
    """Return the explicitly selected TOML path without consuming CLI arguments."""
    ...
    while index < len(argv):
        ...
        if argument == "--config":
            if index + 1 >= len(argv) or argv[index + 1].startswith("-"):
                raise ValueError(
                    "--config requires a path; use --config=<path> for paths "
                    "that begin with '-'"
                )
