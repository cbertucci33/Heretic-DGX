...
class ClusterNode:
    """One physical node in the supported two-node topology."""
...
@dataclass(frozen=True, slots=True)
class ClusterConfig:
    """Validated static configuration for the two-node DGX Spark runtime."""
...
def load_cluster_config(path: str | Path) -> ClusterConfig:
    """Load an exactly-two-node cluster definition and reject ambiguity."""
